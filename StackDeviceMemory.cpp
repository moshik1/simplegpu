...
StackDeviceMemory::Stack::Stack(int device, void* p, size_t size)
  ...
  assert(size == 0 || size >= kSDMAlignment);
...
StackDeviceMemory::Stack::~Stack() {
  ...
  assert(head_ == start_);
  assert(overflowAllocs_.empty());
  assert(overflowSize_ == 0);
...
void* StackDeviceMemory::Stack::getAlloc(
  ...
  // bytes
  assert(size >= kSDMAlignment);
  assert(size % kSDMAlignment == 0);
