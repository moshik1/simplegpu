...
std::vector<float> readFileWithOffset(const std::string& filename, size_t offset, size_t size) {
    ...
    file.seekg(offset);
    std::vector<float> data(size / sizeof(float));  // size should be in bytes
    file.read(reinterpret_cast<char*>(data.data()), size);

    return data;
}

void runFloat32CompressionTest(const std::string& filename, size_t offset, size_t dataSize) {
    try {
        // Read data from file
        ...
        uint32_t maxCompSize = getMaxFloatCompressedSize(FloatType::kFloat32, numElements);
        ...
        auto floatConfig = FloatCompressConfig(
            FloatType::kFloat32,   // Using Float32
        ...
        auto decompressed_dev = res.alloc<float>(stream, numElements);
        ...
        // Verify results
        bool verified = true;
        for (size_t i = 0; i < numElements && verified; ++i) {
            if (std::abs(inputData[i] - decompressed[i]) > 1e-6) {
                verified = false;
                std::cout << "Mismatch at index " << i << ": "
                         << inputData[i] << " vs " << decompressed[i] << std::endl;
            }
        }

        // Print results
        std::cout << "\nFloat32 Compression Benchmark for " << (dataSize / (1024 * 1024)) << "MB (offset: " << offset << "):\n"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (const auto& size : sizes) {
            runFloat32CompressionTest(filename, offset, size);
        }
    } catch (const std::exception& e) {
        std::cerr << "Error: " << e.what() << std::endl;
        return 1;
