...
#include "GpuANSDecode.cuh"
#include "GpuANSUtils.cuh"
...
ANSDecodeStatus ansDecodeBatchSplitSize(
  ...
  // check alignment
  assert(uintptr_t(out_dev) % kANSRequiredAlignment == 0 && "fail in check alignment");
  ...
  for (uint32_t i = 0; i < numInBatch; ++i) {
    ...
    if (i != (numInBatch - 1)) {
      ...
      assert(size % kANSRequiredAlignment == 0 && "check alignment (internal splits affect alignment of things after it)");
    }
