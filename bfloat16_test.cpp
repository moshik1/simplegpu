...
void runBFloat16CompressionTest(const std::string& filename, size_t offset, size_t dataSize) {
    try {
        // Read data from file
        ...
        uint32_t maxCompSize = getMaxFloatCompressedSize(FloatType::kBFloat16, numElements);
        ...
        auto floatConfig = FloatCompressConfig(
            FloatType::kBFloat16,  // Using BFloat16
        ...
        // Print results
        std::cout << "\nBFloat16 Compression Benchmark for " << (dataSize / (1024 * 1024)) << "MB (offset: " << offset << "):\n"
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (const auto& size : sizes) {
            runBFloat16CompressionTest(filename, offset, size);
        }
    } catch (const std::exception& e) {
        std::cerr << "Error: " << e.what() << std::endl;
        return 1;
